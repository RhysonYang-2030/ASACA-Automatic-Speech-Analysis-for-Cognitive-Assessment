"""Utilities for keeping PyTorch's native Windows DLLs discoverable.

This module centralises the logic for registering the ``torch`` wheel's
``.\Lib\site-packages\torch\lib`` directory with the Windows dynamic loader.
The GUI application imports a large portion of PyQt5 before touching
``torch``; Qt (in particular ``QtMultimedia``) calls
``SetDefaultDllDirectories`` internally which clears the process search path
and causes ``LoadLibrary`` to ignore ``PATH`` entries added earlier in the
session.  When that happens the first attempt to import ``torch`` raises
``WinError 1114`` because ``c10.dll`` can no longer locate its dependencies.

By keeping this bootstrap in a shared module we can run it at the very start
of ``app_V2.py`` as well as inside ``inference_V4.py``.  The function is idempotent
and safe to call multiple times; it keeps the returned handles alive so that
the extra directories remain registered for the lifetime of the process.
"""
